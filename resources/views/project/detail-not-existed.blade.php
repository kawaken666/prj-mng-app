<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto mt-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex justify-between items-center bg-fuchsia-400 overflow-hidden shadow-sm sm:rounded-lg p-3 font-bold text-2xl leading-tight">
            ...
            <form method="POST" action="{{ route('project.detail', [$project_detail_not_existed[0]['project_id']]) }}">
                @csrf
                <input type="hidden" name="project_id" value={{ $project_detail_not_existed[0]['project_id'] }}>
            ...
            <div class="py-3">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <hr>
                        <p>{{ $project_detail_not_existed[0]['status'] }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="py-3">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <hr>
                        <p>{{ $project_detail_not_existed[0]['project_overview'] }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-fuchsia-100 p-2 mt-6 sm:rounded-lg">
            ...
            @foreach ($project_detail_not_existed as $tmp_project_detail_not_existed)
                <div class="py-3">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            <h1 class="font-semibold text-xl text-gray-800 leading-tight">{{ $tmp_project_detail_not_existed['name'] }}
                            </h1>
                            <hr><br>
                            <h1 class="font-semibold text-sm text-gray-800 leading-tight">消費工数</h1>
                            <hr>
                            <p>{{ $tmp_project_detail_not_existed['result_man_hour'] }}</p>
                            <br>
                            <h1 class="font-semibold text-sm text-gray-800 leading-tight">状況</h1>
                            <hr>
                            <p>{{ $tmp_project_detail_not_existed['member_overview'] }}</p>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="flex justify-between">
            ...
            <form method="GET" action="{{ route('project.detail.edit', $project_detail_not_existed[0]['project_id'], $project_detail_not_existed[0]['date']) }}">
                @csrf
                <input type="hidden" name="project_id" value={{ $project_detail_not_existed[0]['project_id'] }}>
                <input type="hidden" name="date" value={{ $project_detail_not_existed[0]['date'] }}>
